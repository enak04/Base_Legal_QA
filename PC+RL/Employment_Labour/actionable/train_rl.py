"""
train_rl_v2.py — v2 FIXED: PPO with proper graph recomputation
==============================================================
BUG FIX: "Trying to backward through the graph a second time"
  Root cause: PPO inner loop called loss.backward() on the same
  computation graph across multiple ppo_epochs. The fix is to
  DETACH all stored tensors from the first forward pass and
  RECOMPUTE log-probs fresh inside every PPO inner epoch using
  the stored (state, valid_mask, action_taken) tuples.

Architecture improvements (unchanged from v2):
...
CONFIG = {
    ...
    'use_ppo':              True,
    'ppo_clip_eps':         0.2,
    'ppo_epochs':           2,          # inner PPO update epochs per batch
    # ── GAE ───────────────────────────────────────────────────────────────
    'use_gae':              True,
    'gamma':                0.95,
    'gae_lambda':           0.95,
...
def load_kg(kg_path):
    ...
    if ext == '.pkl':
        ...
        if not required_cols.issubset(df.columns):
            raise ValueError("CSV KG must contain 'source' and 'target' columns")
        ...
        for _, row in df.iterrows():
            ...
            if s in ('source', 'nan', '') or t in ('target', 'nan', ''):
                continue
            rel = str(row.get('relation', 'RELATED')).strip()
            G.add_node(s)
            G.add_node(t)
        ...
        kg_globals = runpy.run_path(kg_path)

        G = nx.DiGraph()

        facts = kg_globals.get('facts', [])
        rules = kg_globals.get('rules', [])
        edges = (
            kg_globals.get('fact_rule_chains')
            or kg_globals.get('chains')
            or kg_globals.get('edges')
            or kg_globals.get('triples')
            or []
        )
        ...
        for item in edges:
            ...
            s = str(s).strip()
            t = str(t).strip()
            rel = str(rel).strip() if rel is not None else 'RELATED'
        ...
        if G.number_of_nodes() == 0 or G.number_of_edges() == 0:
            available = sorted(list(kg_globals.keys()))
            raise ValueError(
                f"Could not build graph from Python KG file: {kg_path}. "
                f"Available variables: {available[:30]}"
            )
...
def train(config):
    ...
    #evice = torch.device(config['device'])
